...
class OverlayTickerService : Service() {
    ...
    private suspend fun buildTickerText(): String {
        val thai = GloRepository.readCached(applicationContext)
        val thaiText = thai?.firstPrize ?: "------"

        return when (val result = SettradeRepository.fetchLiveSetIndex()) {
            is SettradeRepository.FetchResult.Success -> {
                val d = result.data
                "SET ${"%.2f".format(d.set)}   |   Value ${"%,.2f".format(d.value)}   |   2D ${d.twoD}   |   Thai $thaiText   |   Live"
            }
            is SettradeRepository.FetchResult.Failure -> {
                "SET --   |   Value --   |   Thai $thaiText   |   ခဏစောင့်ပါ..."
            }
        }
    }
